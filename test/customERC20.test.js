...
contract("customERC20", (accounts) => {
  ...
  it("approve, allowance & transferFrom", async () => {
    ...
    await instance.transferFrom(accounts[0], accounts[2], 100, {
      from: accounts[1],
    });

    let _allowance_after_transfer = await instance.allowance(
      accounts[0],
      accounts[1]
    );
